...
caught in the page and forwarded. But an ordinary cross-document link click fires _both_ a
...
The naive approach — `chrome.tabs.update({ url })` — _appends_ a history entry. Jump back
...
`navigate` event and echoes it back, letting the worker distinguish _"I caused this, verify
where it landed"_ from _"the user did this, move the cursor."_ Chrome exposes no first-class
...
narrow the discriminant _before_ the runtime check that validates it.
