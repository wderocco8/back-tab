...
**Prerequisite:** [0003](0003-jump-to-existing-node.md) — without it, every shortcut press
grows the snake, which is worse than the popup case because the shortcut is meant to be
used constantly.
...
**Use `chrome.commands`, not an injected keydown handler.** It works regardless of page focus, never
fights with a site's own shortcuts (Gmail, Notion), and — important for
[0006](0006-permission-diet.md) — **a commands keypress grants `activeTab`**, which is what makes
the `history.go()` injection legal without host permissions. Chrome allows 4 commands with suggested
key bindings; spend two on next/prev sibling.
